    ...
    <div style={styles.container}>
      <div style={styles.card}>
        ...
        <input
          ...
          disabled={isLoading}
        />
        <button style={{...styles.button, opacity: isLoading ? 0.6 : 1}} onClick={handleLogin} disabled={isLoading}>
          {isLoading ? "Logging in..." : "Login"}
        </button>
        {message && (
          <div style={{...styles.message, color: type === "success" ? "#4caf50" : "#ff4d4d"}}>
            {message}
          </div>
        )}
        <p style={styles.text}>New user? <span style={styles.link} onClick={showSignup}>Sign Up</span></p>
      </div>
    </div>
  );
}
...
const styles = {
  ...
  heading: { marginBottom: "25px", fontSize: "26px", fontWeight: "600" },
  input: {
    width: "100%",
    padding: "14px",
    margin: "12px 0",
  ...
  message: { marginTop: "15px", fontWeight: "bold", fontSize: "14px" },
